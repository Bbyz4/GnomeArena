    ...
    public void displayItems(List<Pair<Item, Pair<Integer,Integer>>> items, Stage stage)
    {
        for(int i=0; i<items.size(); i++)
        {
            ...
            else if(it instanceof WeaponPickup)
            {
                ...
                spr = new Sprite(GameConfig.WEAPON_SKINS.get(wpit.spawnedWeapon.getClass().getSimpleName()));
            }
            else
            {
                spr = new Sprite(GameConfig.ITEM_SKINS.get(it.getClass().getSimpleName()));
            }

            Image im = new Image(spr);
            im.setOrigin(im.getWidth()/2, im.getHeight()/2);
            Pair<Integer,Integer> imagePos = new Pair<>(items.get(i).getValue().getKey(), items.get(i).getValue().getValue());
            im.setPosition(getItemsXPos(imagePos.getKey(), im.getWidth()), getItemsYPos(imagePos.getValue(), im.getHeight()));
            im.setScale(GameConfig.BOARD_IMAGES_SCALE*totalBoardScale);
            stage.addActor(im);
        }
    }
}
